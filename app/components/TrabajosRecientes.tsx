                ...
                aria-hidden="true" className='pointer-events-none absolute inset-x-0 -z-10 transform-gpu overflow-hidden blur-2xl -right-30 -top-52'>
                    ...
                    <div className=' flow-root'>
                        <div className=' rounded-xl bg-gray-900/5 ring-1 ring-inset ring-gray-900/5 '>
                                ...
                                <div className="mt-6 grid grid-cols-4 sm:gap-32 xl:gap-5 p-5 border bg-gray-500/10 border-gray-300 hover:shadow-xl transition duration-300 rounded-lg ">
                                    {data.map((job) => (
                                    <div key={job._id} className="group relative border border-gray-300  flex flex-col items-center justify-between md:px-32 sm:px-16 h-cardJobSize  rounded-lg hover:shadow-xl transition duration-300 bg-gray-500/20 z-10 xl:px-5">
                                        <div className="aspect-square md:w-32 w-32 h-44 m-5 overflow-hidden rounded-lg ">
                                        ...
                                        <div className="py-5 flex flex-col justify-between md:w-52 sm:w-16">
                                            <div className="flex flex-col items-center gap-3">
                                                <h3 className="text-white text-lg justify-between">
                                                    {job.name}
                                                </h3>
                                                <p className="text-lg text-blue-400">{job.categoryName}</p>
                                                <Link href={`/job/${job.slug}`}>
                                                    <Button className="px-10 sm:px-5 text-white bg-blue-500">Ver Trabajo</Button>
                                                </Link>
                                            </div>
                                        </div>
                                    </div>
